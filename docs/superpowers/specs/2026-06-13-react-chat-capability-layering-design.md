...
| 3 轮收尾 | Chat 达到 3 轮仍有 `tool_calls` 时,注入 system 提示"工具预算已用尽,请基于已有信息直接作答",再走一轮(无工具)收尾 |
...
| 达上限行为 | 注入提示 → 作答收尾 | 正常 100 轮 | 正常 100 轮 |
...
**(d) 达上限收尾** —— 循环内,当一轮的 `finishReason === 'tool_calls'` 但已到 `maxRounds - 1`(Chat 的第 3 轮)时,不再执行工具,而是 push 一条 system 消息:

```
工具预算已用尽,请基于已有信息直接作答,不要再调用工具。
```

然后 `continue` 到下一轮 —— 该轮 LLM 看到提示后给出最终作答,`finishReason` 非 `tool_calls`,正常 `return`。为保证这一收尾轮有空间,循环实际可迭代到 `maxRounds + 1`(多预留 1 轮纯作答),或单独在循环后追加一轮。
...
- **3 轮收尾轮**:注入的 system 提示要求"不要再调用工具";若模型仍执意返回 `tool_calls`,以循环上限自然截断,把当前内容作为最终输出(降级为硬截断,但概率极低)。
...
4. **3 轮收尾**:Chat 第 3 轮工具返回后注入 system 提示,且下一轮无 `tool_calls`,用户拿到完整作答。
